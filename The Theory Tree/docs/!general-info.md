...
Beyond that, the main way to add content is through creating layers, often in [theory.js](/js/layers/theory.js). You can add new layers by calling `addLayer(layername, layerdata)`. There is an example of a basic layer in [theory.js](/js/layers/theory.js) showing the recommended method. It is just an example and can be freely deleted. You can also use it as a reference or a base for your own layers. 
